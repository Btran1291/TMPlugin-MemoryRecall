...
async function rerankWithCohere(query, pineconeResults, userSettings) {
  const { cohereAPIKey, cohereRerankModel, cohereTopN, cohereMaxTokensPerDoc } = userSettings;
  ...
  const requestBody = { "model": model, "query": query, "documents": pineconeResults.map(match => match.metadata.content), "top_n": topN, "max_tokens_per_doc": maxTokens };
  try {
    ...
    if (data && data.results) {
      return data.results.map(result => {
        ...
        return {
          ...
          metadata: {
            message_number: match.metadata.message_number,
...
async function recall_chat_memory(params, userSettings) {
  const { query } = params;
  const { pineconeAPIKey, pineconeIndexHostURL, topK, pineconeAPIVersion, namespace, enableRerank, similarityMetric } = userSettings;
  ...
  try {
    ...
    if (data && data.matches) {
      if (rerankEnabled) {
        return await rerankWithCohere(query, data.matches, userSettings);
      } else {
        return data.matches.map(match => {
          return {
            relevance: normalizeRelevanceScore(match.score, metric, data.matches),
            metadata: {
              message_number: match.metadata.message_number,
              role: match.metadata.role,
              content: match.metadata.content
      ...
      return "No matching results found in Pinecone.";
    }
  } catch (error) {
